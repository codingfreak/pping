...
3. Calculate the new Hash:
    ...
    ```
    $hashExe = Get-FileHash .\bin\Release\net10.0\publish\pping.exe | Select -ExpandProperty Hash 
    $hashDll = Get-FileHash .\bin\Release\net10.0\publish\pping.dll | Select -ExpandProperty Hash 
    ```
...
8. Push choco:
    ...
    ```
    choco push .\bin\Release\net10.0\win-x64\publish\pping.$version.nupkg
    ```
